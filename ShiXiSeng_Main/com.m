...
@interface com()<UIGestureRecognizerDelegate>
...
-(IBAction)pressFix:(id)sender{
    if ([_fix.titleLabel.text isEqualToString:@"已关注"]) {
        
        [_fix setTitle:@"未关注" forState:UIControlStateNormal];
...
- (void)setUp
{
    ...
    [self .deleteButton setImage:[UIImage imageNamed:@"io"] forState:UIControlStateNormal];
...
{
       CGFloat width = roundf(50+ 2. * self.menuOptionButtonTitlePadding);
...
- (void)setMenuOptionsViewHidden:(BOOL)hidden animated:(BOOL)animated completionHandler:(void (^)(void))completionHandler
{
    ...
    [UIView animateWithDuration:(animated) ? self.menuOptionsAnimationDuration : 0.
                     ...
                     animations:^
     {
         ...
         if (!hidden) {
             [self.delegate contextMenuDidShowInCell: (id)self];
         } else {
             [self.delegate contextMenuDidHideInCell: (id)self];
         }
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer;
{
    if ([recognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (recognizer.state == UIGestureRecognizerStateBegan) {
            ...
            if (velocity.x > 0) {
                [self.delegate contextMenuWillHideInCell:(id)self];
            } else {
                [self.delegate contextMenuDidShowInCell:(id)self];
            }
        } else if (recognizer.state == UIGestureRecognizerStateChanged) {
            CGPoint velocity = [recognizer velocityInView:self.contentView];
            if (!self.contextMenuHidden || (velocity.x > 0. || [self.delegate shouldShowMenuOptionsViewInCell:(id)self])) {
                if (self.selected) {
                    [self setSelected:NO animated:NO];
                }
...
- (void)deleteButtonTapped
{
    if ([self.delegate respondsToSelector:@selector(contextMenuCellDidSelectDeleteOption:)]) {
        [self.delegate contextMenuCellDidSelectDeleteOption:(id)self];
    }
}
