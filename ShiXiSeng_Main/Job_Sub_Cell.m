...
//  ShiXiSeng_Main
//
//  Created by 是 撒 on 5/15/15.
//  Copyright (c) 2015 jackson. All rights reserved.
//

#import "Job_Sub_Cell.h"

@interface Job_Sub_Cell()<UIGestureRecognizerDelegate>
...
- (void)setUp
{
    ...
    self.editable = YES;
    self.moreOptionsButtonTitle = @"Edit";
    ...
    if ([recognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (recognizer.state == UIGestureRecognizerStateBegan) {
           ...
           //     [self.delegate contextMenuWillHideInCell:self];
            ...
            CGPoint velocity = [recognizer velocityInView:self.contentView];
            if (!self.contextMenuHidden || (velocity.x > 0. )) {
                if (self.selected) {
                    [self setSelected:NO animated:NO];
                }
