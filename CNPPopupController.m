...
- (instancetype)initWithTitle:(NSString*)text
    ...
    if (self) {
            ...
            datepicker=[[UIDatePicker alloc] initWithFrame:CGRectMake(0, 30, 120, 200)];
...
- (void)setUpPopup {
    ...
    if (_flag == 1 || _flag == 2 || _flag == 3 || _flag == 4 ||_flag == 0 || _flag ==9) {
        title1 = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 30)];
        [title1 setText:_text];
        title1.font = [UIFont systemFontOfSize:18];
        [title1 setTextAlignment:NSTextAlignmentLeft];
        [title1 setTextColor:[UIColor colorWithRed:0.0/255 green:161.0/255  blue:233.0/255 alpha:1.0f]];
        [title1 setBackgroundColor:[UIColor colorWithRed:222.0/255 green:251.0/255  blue:255.0/255 alpha:1.0f]];
...

{
    ...
    if (_flag == 1) {
        ...
        if(_delegate!=nil && [_delegate respondsToSelector:@selector(didSelect:)]){
            [_delegate didSelect:destDateString];
            return;
        }
        ...
        NSString *destDateString = [dateFormatter stringFromDate:selected];
        NSLog(@"%@",destDateString);
        if(_delegate!=nil && [_delegate respondsToSelector:@selector(didSelect1:)]){
            [_delegate didSelect1:destDateString];
            return;
        }
        ...
        NSString *destDateString = [dateFormatter stringFromDate:selected];
        NSLog(@"%@",destDateString);
        if(_delegate!=nil && [_delegate respondsToSelector:@selector(didSelect2:)]){
            [_delegate didSelect2:destDateString];
            return;
        }
